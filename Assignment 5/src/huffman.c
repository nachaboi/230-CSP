...
static void compute_freq (FILE *fp, Context *ctx) {
  ...
  for(int i = 0; i < NUMBER_OF_CHARS; i++) {
    ctx->table[i].v = 0;
    ctx->table[i].c = i; //idk how to do this
  }
 ...
 * table.  Each ne1w TreeNode object is enqueued into the priority queue.
 */
static void create_tree_nodes (Context *ctx) {
  ...
  for(int i = 0; i < NUMBER_OF_CHARS; i++) {
    if(ctx->table[i].v > 0) {
      TreeNode *newNode = tree_new();
      newNode->type = LEAF;
      newNode->freq.v = ctx->table[i].v;
      newNode->freq.c = ctx->table[i].c;
      pqueue_enqueue(ctx->pq, newNode);
    }
  }
