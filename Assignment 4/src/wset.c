...
int wset_add (WordSet *wset, char *word) {
  ...
  WordNode **a = search(wset, word);
  if((*a) == NULL) {
    //!=
    // int temp = (*a)->freq;

    (*a) = node_new(word);
    (wset->size) = (wset->size) + 1;
  }
...
int wset_fast_add (WordSet *wset, char *word) {
  ...
  WordNode **a = search(wset, word);
  if((*a) == NULL) {
    // WordNode *newWord = node_new(word);
    // WordNode *theCur = wset->head;
    // newWord->next = theCur;

    newWord = node_new(word);
    newWord->next = wset->head;
    wset->head = newWord;
    (wset-> size) = (wset->size) + 1;
  }
  else {
