...
char *make_block (int block_size) {
  ...
  //
  char *arr = (char*) malloc(sizeof(char) * block_size);
  int i;
...
Line *make_lines (int line_count, int block_size) {
  ...
  //
  Line *arr = (Line*) malloc(sizeof(Line) * line_count);
  int i;
  for(i=0; i<line_count; i++) {
    arr[i].valid = 1;
    arr[i].tag = 0;
    arr[i].accessed = (char*) malloc(sizeof(char) * 8);
    arr[i].block_size = block_size;
  }
...
Set *make_sets (int set_count, int line_count, int block_size) {
  ...
  //
  Set *arr = (Set*) malloc(sizeof(Set)*set_count);
  int i;
  for(i=0; i<set_count; i++) {
    arr[i].lines = make_lines(line_count, block_size);
    arr[i].line_count = line_count;
    // arr[i].lru_queue = NULL;
  }

  make_lines(line_count, block_size);
  return arr;
}
...
Cache *make_cache (int set_bits, int line_count, int block_bits) {
  ...
  make_block(exp2(block_bits));
  make_lines(line_count, exp2(block_bits));
  make_sets(exp2(set_bits), line_count, exp2(block_bits));
